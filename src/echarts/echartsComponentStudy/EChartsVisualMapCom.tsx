...
export default defineComponent({
    components: { Base },
    setup() {
        const css: CSSProperties = { width: '450px', height: '450px', border: 'solid 1px #000' }
        const visualMap_continuous: EChartOption.VisualMap = {
            type: 'continuous',
            // id?: string;
            min: 0,//指定组件的允许的最小值。'min' 必须用户指定。
            max: 100,//指定组件的允许的最大值。'max' 必须用户指定。
            range: [30, 90],//指定手柄对应数值的位置。range 应在 min max 范围内。
            calculable: true,//是否显示拖拽用的手柄（手柄能拖拽调整选中范围）。
            realtime: true,//拖拽时，是否实时更新。
            inverse: false,//是否反转 visualMap 组件。
            precision: 0,//数据展示的小数精度，默认为0，无小数点。
            itemWidth: 10,//控制控件的宽度，即长条的宽度
            itemHeight: 100,//控制控件的高度，即长条的高度。
            align: 'bottom',//指定控制控件中展示条相对手柄和文字的摆放位置//可选值:'auto' | 'left' | 'right' | 'top' | 'bottom';
            text: ['a', 'b'],//设置控制控件两头的说明文本
            textGap: 20,//设置控制控件两头的说明文本与控制控件的间隔
            show: true,//是否显示 visualMap-continuous 组件。如果设置为 false，不会显示，但是数据映射的功能还存在。
            dimension: 2,//指定用数据的『哪个维度』，映射到视觉元素上。『数据』即 series.data。 可以把 series.data 理解成一个二维数组 //string | number;
            seriesIndex: 0,//指定取哪个系列的数据，即哪个系列的 series.data。 默认取所有系列。// number | number[];
            hoverLink: true,//鼠标悬浮到 visualMap 组件上时，鼠标位置对应的数值 在 图表中对应的图形元素，会高亮。
            inRange: {
                color: ["#313695", "#4575b4", "#74add1", "#abd9e9", "#e0f3f8", "#ffffbf", "#fee090", "#fdae61", "#f46d43", "#d73027", "#a50026"],
                //symbol:['circle','rect','roundRect'],//'circle', 'rect', 'roundRect', 'triangle', 'diamond', 'pin', 'arrow', 'none'
            },
            ...
            // zlevel?: number;
            // z?: number;
            left: 'center',//number | string;
            top: 10, //number | string;
            // right?: number | string;
            // bottom?: number | string;
            orient: 'horizontal',//设置控制控件的摆放方式 //默认自调配,可选值:'vertical' | 'horizontal';
        ...
        const visualMap_piecewise: EChartOption.VisualMap = {
            type: "piecewise",
            // id?: string;
            splitNumber: 8,//对于连续型数据，自动平均切分成几段。默认为5段。 连续数据的范围需要 max 和 min 来指定. 如果设置了 visualMap-piecewise.pieces 或者 visualMap-piecewise.categories，则 splitNumber 无效。
            //pieces:[],//自定义『分段式视觉映射组件（visualMapPiecewise）』的每一段的范围，以及每一段的文字，以及每一段的特别的样式 //PiecesObject[];
            // categories?://用于表示离散型数据（或可以称为类别型数据、枚举型数据）的全集。// string[];
            min: 0,
            max: 100,
            ...
            selectedMode: 'multiple',//可选值:'multiple' | 'single';
            //selected:{},//用于初始化各段的选中状态,默认都选中
            // inverse?: boolean;
            // precision?: number;
            // itemWidth?: number;
            // itemHeight?: number;
            ...
            // itemSymbol?: string;
            // show?: boolean;
            // dimension?: string | number;//指定用数据的『哪个维度』，映射到视觉元素上。『数据』即 series.data。 可以把 series.data 理解成一个二维数组
        ...
        const option_continuous: EChartOption = {
            tooltip: {},
            xAxis: {
                type: "category",
            ...
            visualMap: [visualMap_continuous],
            series: [{
                name: "Gaussian",
                type: "heatmap",
                data: [
        ...
        const option__piecewise = { ...option_continuous }
        option__piecewise.visualMap = [visualMap_piecewise]
        return () => (
            <div style={{ display: 'flex', flexDirection: 'row' }}>
                <Base option={option_continuous} style={{ ...css, marginRight: '10px' }} />
                <Base option={option__piecewise} style={css} />
            </div>
        )
    }
})